...
OUTPUTTAR="jetTriggerAnalysis.tar.gz"
...
sed -i '' 's/GITHASHHERE/'${GITHASH}'/' triggerEfficiencyAnalysis.cxx

# Make sure there are no object files going to the tar
make clean

# Create the new tar ball
tar -cvzf $OUTPUTTAR Makefile triggerEfficiencyAnalysis.cxx src

# Put placeholder string back to the main analysis file
sed -i '' 's/'${GITHASH}'/GITHASHHERE/' triggerEfficiencyAnalysis.cxx
